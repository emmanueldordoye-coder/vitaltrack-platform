...
7. Resets the staging database to a clean state.
8. Runs `supabase db push --dry-run`.
9. Applies all migrations to staging.
10. Runs the Dentira seed file twice to confirm idempotency.
11. Confirms these tables exist:
   - `products`
   - `categories`
   - `manufacturers`
   - `vendors`
   ...
   - `receiving_events`
12. Confirms Row Level Security is enabled on the expected tenant tables.
13. Confirms foreign keys and expected indexes exist.
14. Confirms Dentira seed counts and low-stock detection.
15. Runs `database/validation/005_project_lighthouse_security_validation.sql` to prove cross-tenant access is rejected, same-tenant manager workflow still succeeds, receiving events are append-only, and `received_by` cannot be spoofed.
...
| `failed to connect to postgres` or `password authentication failed` | Incorrect database password or project ref. | Verify `STAGING_SUPABASE_PROJECT_REF` and `STAGING_SUPABASE_DB_PASSWORD`. |
